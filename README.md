# 8848 股票策略分析器

基于"8848"交易体系的 A 股分析 Web 应用。支持实时行情分析、AI 辅助决策、批量自选股管理、持仓管理、技术指标和历史数据回溯。

## 功能

- **8848 压力/支撑线**：基于日内均价计算上压线 (`avg / 0.98848`) 和下支线 (`avg * 0.98848`)
- **策略信号**：持仓模式（动态止盈）+ 观察模式（Fibonacci 38.2% / 61.8% / 78.6% 回撤位）
- **AI 分析**：流式输出，支持 Claude / OpenAI（含 DeepSeek）/ Gemini，可发起多轮追问
- **Tushare 工具调用**：AI 可自主调用13个工具（资金流向、龙虎榜、融资融券、技术指标、筹码分布等）获取数据后再分析
- **批量分析**：一键分析全部自选股，结果保存30分钟可随时查看
- **自选股管理**：添加/删除股票，支持行业 tag 标注和按行业过滤
- **分钟级盯盘**：仅监控手动勾选的股票，支持关键价、5分钟异动和分钟放量提醒
- **次日计划**：批量调用一次 AI 生成多只股票的结构化计划，确认后按交易日自动执行
- **盘中校准**：09:35 确定性校准，10:00、13:05 和 14:35 批量 AI 校准，保留完整修订记录
- **网页通知中心**：SSE 实时弹窗、未读计数、断线补发、行情延迟保护和盘后效果统计
- **持仓管理**：记录成本价、阶段高低点，自动追踪最高价以辅助止盈
- **历史数据**：60日 OHLC + 技术指标（MACD、BOLL、RSI、移动筹码、揉搓线）
- **交易日志**：记录每笔操作并可事后复盘

## 快速开始

```bash
cp .env.example .env   # 填入 token
uv sync                # 安装依赖
./start.sh             # 后台启动（端口 8848）
```

或前台运行：

```bash
uv run uvicorn app:app --host 0.0.0.0 --port 8848
```

打开浏览器访问：[http://localhost:8848](http://localhost:8848)

停止服务：

```bash
./stop.sh
```

## 次日盯盘使用方法

盯盘开关只用于确定监控范围，**勾选不等于已经生成或启用计划**。完整流程如下：

1. 在首页自选股列表或批量分析页打开股票的“盯盘”开关。
2. 进入“查看详情”，点击“刷新信号”，获取最新批量分析数据。
3. 点击“AI生成次日计划”。系统只选择已打开盯盘开关的股票，并在一次批量调用中生成计划，避免逐只重复注入 Skills 提示词。
4. 检查生成的草稿。关键价和确认分钟可以手动修改，修改后点击对应规则的“保存”。
5. 点击“确认并启用全部草稿”。计划会绑定到 Tushare `trade_cal` 返回的下一交易日，自动跳过周末和法定休市日。

计划启用后无需手动启动。到了对应交易日，后台会自动执行：

| 时间 | 任务 | AI 调用 |
|---|---|---|
| 盘中每分钟 | 检查突破、跌破、接近关键价、5分钟异动和分钟放量 | 否 |
| 09:35 | 处理大幅高低开、开盘直接破位和追涨风险 | 否 |
| 10:00 | 判断上午方向、突破质量和量价配合 | 一次批量调用 |
| 13:05 | 根据上午完整走势校准午后计划 | 一次批量调用 |
| 14:35 | 尾盘隔夜决策：结合持仓和可用现金判断买入、补仓、持有、减仓或放弃机会 | 一次批量调用 |
| 收盘后 | 计划过期，统计提醒后的最高收益和最大回撤 | 否 |

盘中 AI 校准只能继续、暂停、判定计划失效或小幅修正规则。后端会拒绝修改已触发规则、超过 1% 的关键价调整，以及向下放宽风险线。原始规则、校准原因和实际修改都会保留在“今日计划校准时间线”中。

自选股行情按批次拉取后统一执行规则扫描，避免逐只网络请求导致分钟确认失真。价格规则满足确认条件时会写入盯盘通知；盘中校准产生 `pause_opportunity`、`tighten_risk` 或 `invalidate` 时也会写入通知，并标记为“校准”事件。页面关闭不影响后台执行，重新打开后会回补最近通知。

### 运行条件和提醒行为

- 对应交易日盘中，Web 服务必须保持运行。
- 网页打开时，通过 SSE 立即显示提醒弹窗。
- 网页关闭不影响后台判断和事件落库；重新打开后，通知中心会补拉遗漏事件。
- 行情超过 150 秒没有更新时，系统暂停产生新提醒，避免使用陈旧价格误报。
- 通知会附带指标线名称、当前线值、当前价和分时均价。分时均价/黄线使用盘中动态值，不沿用计划生成时的旧值。
- `near` 买卖规则先提示“到达观察区”，随后在价格越过关键线 0.2% 并持续满足确认分钟数后，另发明确操作确认；仅靠近不会冒充站稳。
- 同一规则解除后会重新布防；确认分钟和 0.2% 滞回区间用于避免关键位附近反复穿越刷屏。
- 突破规则触发后另行跟踪价格推进：较触发价（之后为上次状态提醒价）上涨至少 1%，且连续 3 个分钟快照确认，发送“持续走强”状态提醒；后续提醒至少间隔 15 分钟，并必须再次推进至少 1%。分钟缺口和午休会重置连续确认，不重复计算同一分钟。以上仅为通知阈值，不是新买卖条件。
- “持续走强”不更改原规则、不推断成交，也不提供成交或停用按钮；部分成交和已完成买入后仍可观察，持仓动作在空仓后停止观察，停用、暂停、忽略或稍后期间不发此类提醒。实际成交请在原规则提醒中记录。升级前已触发的规则从升级后首次有效快照建立基准，不补发过去的上涨。
- AI 校准通知明确区分建议与执行：列出后端实际接受的规则调整；没有调整时显示“未修改任何规则”。`tighten_risk` 意见不等于已收紧风险线，更不等于规则刚刚触发或已经成交。
- 关闭某只股票的盯盘开关后，其规则停止参与扫描，但历史计划和提醒记录不会删除。

通知中心会显示“行情正常”“行情延迟”或“当前休市”。批量分析页会分别显示今日执行计划、下一交易日草稿和当日校准时间线。

> 建议在收盘后生成并确认下一交易日计划。如果只勾选开关而没有生成、确认计划，次日不会执行任何规则。

### 本地系统通知监听器

浏览器页面关闭后仍需接收通知时，可在本地电脑的根目录 `.env` 中设置：

```env
WATCH_SERVER_URL=http://服务器IP:8848
WATCH_NOTIFY_POLL_SECONDS=15
```

启动监听：

```bash
uv run python scripts/watch_notify.py
```

脚本通过 SSE 实时接收事件，并定时调用通知 API 补漏；游标独立保存在
`~/.local/state/8848-watch/last_event_id`，不会替网页通知中心标记已读。首次运行默认忽略既有事件，
需要回放时使用 `--replay`。macOS 可先运行：

```bash
uv run python scripts/watch_notify.py --test-notification
```

公网 `HTTP + IP` 会明文传输提醒内容，且当前 Web 应用本身没有访问鉴权。短期使用至少应通过防火墙限制来源 IP；长期建议改为 HTTPS、SSH 隧道或 VPN。

## 环境变量（`.env`）

| 变量 | 默认值 | 说明 |
|---|---|---|
| `TUSHARE_TOKEN` | — | Tushare Pro token，历史数据必填 |
| `COMMON_STOCK_CODES` | — | 自选股代码，逗号分隔，如 `600519,000001` |
| `WATCH_SERVER_URL` | — | 本地通知监听器访问的线上服务地址 |
| `WATCH_NOTIFY_POLL_SECONDS` | `15` | 本地监听器补漏轮询间隔（秒，最小 5） |
| `AI_PROVIDER` | `claude` | `claude` / `openai` / `gemini` |
| `CLAUDE_API_KEY` / `CLAUDE_MODEL` / `CLAUDE_BASE_URL` | — | Claude 配置 |
| `OPENAI_API_KEY` / `OPENAI_MODEL` / `OPENAI_BASE_URL` | — | OpenAI / DeepSeek / Ollama 配置 |
| `OPENAI_MAX_TOKENS` | `16384` | OpenAI 兼容接口的 max_tokens |
| `GEMINI_API_KEY` / `GEMINI_MODEL` | — | Gemini 配置 |

## 历史数据拉取

```bash
uv run python fetch_history.py             # 最近 60 个交易日
uv run python fetch_history.py --backfill  # 最近 90 日（首次初始化）
uv run python fetch_history.py --codes 600519,588170  # 指定股票/ETF
```

ETF 自动识别（沪市5开头、深市1开头）并走 `fund_daily` 接口。脚本同时拉取三大指数（上证、深证、创业板）用于 AI 市场参照。

收盘后自动拉取（crontab）：

```
35 15 * * 1-5 cd /path/to/MeanDeviation-Web && uv run python fetch_history.py >> fetch_history.log 2>&1
```

## 传统 Python/Pip 方式

```bash
pip install -r requirements.txt
uvicorn app:app --host 0.0.0.0 --port 8848
```
