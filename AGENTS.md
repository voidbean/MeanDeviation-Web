...
**8848 股票策略分析器** — A FastAPI web app for A-share (Chinese stock market) analysis based on the "8848" trading strategy. The app fetches real-time quotes via Tushare, calculates pressure/support levels, applies Fibonacci retracement signals, and optionally calls an LLM (Codex/OpenAI/Gemini) for AI-driven trading advice.
...
| `AI_PROVIDER` | `Codex` | `Codex` / `openai` / `gemini` |
| `CLAUDE_API_KEY` / `CLAUDE_MODEL` / `CLAUDE_BASE_URL` | — | Codex config |
...
- Codex: `input_schema`, loop exits on `stop_reason == "end_turn"`
